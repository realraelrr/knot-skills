...
- 生成 2-3 张配图时,必须先绑定原始版式槽位:单张大图 = S22;多图 = S15/S16 网格改造;不要使用未登记图文结构
...
1. 同时打开三份页面:`assets/template-swiss.html`、当前生成页、正在修改的测试 deck。
2. 截图前先等入场动效稳定(约 1-2 秒)。不要把动画中间态误判成"内容缺失"或"版式空白"。
3. 先看视觉:标题重量、头部距离、图片落位、底部安全区、caption 是否被 nav 挡住。
4. 对照 `assets/template-swiss.html` 的同类版式,不要只对照 CSS helper;以实际页面结构和视觉结果为准。
5. 再回到代码,检查该页是否误用了不属于该版式的组件,例如把未登记图片结构塞进正文页,或把 P7 图表用于没有真实数值的概念列表。
...
未登记图文结构不可用于默认 Swiss deck。需要单张大图时使用 S22;需要 2-3 张同类图片时用 S15/S16 的原始网格骨架改造成图片格。
...
| 单图解释论点 / 图文混排 | S22 Image Hero, or S08 Duo Compare when text and image have equal weight |
| 2-3 张图片/截图/图表证据链 | S15 Image Matrix / S16 Multi-card Brief image adaptation |
...
| 1 张核心图片 + 一段解释 | S22 Image Hero, or S08 Duo Compare when text and image have equal weight | 未登记图文结构 |
| 2-3 张同类图片 | S15 Image Matrix / S16 Multi-card Brief image adaptation | 未登记图文结构 |
